            ...
            >
                ...
                <div className="p-6 space-y-6">
                        ...
                        {newUser.password && (
                            <div>
                                <div className="flex justify-between items-center mb-1">
                                    ...
                                    <span className={clsx(`text-sm font-bold`, passwordStrength.color.replace('bg-', 'text-'))}>{passwordStrength.label}</span>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div className={clsx(`h-2 rounded-full`, passwordStrength.color)} style={{ width: `${(passwordStrength.score / 5) * 100}%` }}></div>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                            {passwordRequirements.map((req) => (
                                <div key={req.key} className="flex items-center gap-1.5 text-sm">
                                    ...
                                    <span className={clsx(req.met && "text-gray-500 line-through")}>{req.label}</span>
                                </div>
                            ))}
                        </div>
                    </fieldset>
                    ...
                    <fieldset className="bg-gray-50 rounded-lg p-4 flex items-center justify-between">
                        ...
                        <button
                            type="button"
                            onClick={() => handleChange('is_admin', !newUser.is_admin)}
                            className={clsx(
                                'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2',
                                newUser.is_admin ? 'bg-blue-600' : 'bg-gray-200'
                            )}
                        >
                            ...
                            <span
                                className={clsx(
                                    ...
                                    newUser.is_admin ? 'translate-x-5' : 'translate-x-0'
                                )}
                            />
                        </button>
                    </fieldset>
