...
CREATE TABLE public.kyte_conversions (
    ...
    error_message text,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE(company_id, kyte_order_number)
);
COMMENT ON TABLE public.kyte_conversions IS 'Tracks Kyte to QuickBooks conversion history.';
CREATE INDEX ON public.kyte_conversions(company_id);
CREATE INDEX ON public.kyte_conversions(status);
CREATE INDEX ON public.kyte_conversions(created_at DESC);
